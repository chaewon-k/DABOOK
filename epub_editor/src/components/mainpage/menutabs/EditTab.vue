  ...
  <v-tabs class="ml-3" show-arrows v-model="tab">
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <v-btn class="align-self-center rounded-sm" v-on="on" icon medium @click="edit('undo')"
    ...
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <v-btn class="align-self-center rounded-sm" v-on="on" icon medium @click="edit('redo')"
    ...
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <v-btn class="align-self-center rounded-sm" v-on="on" icon medium @click="edit('cut')"
    ...
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <v-btn class="align-self-center rounded-sm" v-on="on" icon medium @click="edit('copy')"
    ...
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <v-btn class="align-self-center rounded-sm" v-on="on" icon medium @click="edit('paste')"
    ...
    <v-dialog v-model="replaceDialog" max-width="400">
      <v-card>
        <DialogTitle title="edit-replace" @toggle-dialog="replaceDialog = false" />
  ...
  methods: {
    ...
    findUp: function() {
      if (this.findTextIndex == 0) {
        eventBus.$emit('setCursor', this.findTextArray[0], this.findText.length);
        return;
      } else {
        eventBus.$emit('setCursor', this.findTextArray[--this.findTextIndex], this.findText.length);
      }
    },
    ...
    replace: function() {
      ...
      if (this.$store.state.selectedFileDirectory !== '') {
        ...
        this.$store.dispatch('setAlertMessage', 'success.replace');
        this.findText = '';
        this.replaceText = '';
        this.$refs.findTextInput.resetText();
        this.$refs.replaceTextInput.resetText();
      } else {
        this.$store.dispatch('setAlertMessage', 'select-text');
      }
    },
